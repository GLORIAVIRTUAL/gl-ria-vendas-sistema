...
async function buscarProximoHorarioDisponivel(base44, diasParaBuscar = 14) {
  ...
  for (let i = 0; i < diasParaBuscar; i++) {
    ...
    for (const horario of HORARIOS_DISPONIVEIS) {
      ...
      // Se for hoje, pula horários que já passaram (com margem de 1 hora)
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (req.method === 'POST') {
      ...
      // Se usar_proximo_horario = true, busca automaticamente
      ...
      // Valida se data e horário foram definidos
      ...
      if (agendamentosExistentes.length > 0) {
        // Busca próximo disponível automaticamente
        const proximo = await buscarProximoHorarioDisponivel(base44);
        return Response.json({
          sucesso: false,
