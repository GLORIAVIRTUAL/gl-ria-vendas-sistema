...
export async function pararCadenciaPorResposta(db: any, phone: string, texto = '') {
  ...
  if (leadId) {
    // Resposta comercial positiva → Engajado.
    await moverEstagio(db, leadId, 'Engajado', {
      origem: 'WhatsApp',
      motivo: 'Prospect respondeu à cadência de prospecção',
      prospect_id: prospect.id
    });
  }
  if (!leadId) {
    ...
    await registrarHistorico(db, lead, '', 'Engajado', {
      origem: 'WhatsApp',
      motivo: 'Lead criado após resposta à cadência',
      prospect_id: prospect.id
    });
  }
