...
import ProspectScore from "@/components/prospeccao/ProspectScore";
...
export default function ProspectCard({ prospect, saved, onSave, onAddCRM, onAnalisar, onWhatsApp, onEmail, busy }) {
  ...
  return <Card className="border-slate-500/40 bg-slate-950/55"><CardContent className="space-y-4 p-5">
    ...
    {saved && <ProspectScore prospect={prospect} />}
    {saved && <Button variant="outline" disabled={busy} onClick={onAnalisar}>{busy ? "Analisando..." : prospect.analisado_em ? "Reanalisar com IA" : "Analisar com IA"}</Button>}
