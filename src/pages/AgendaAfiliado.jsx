...
export default function AgendaAfiliado() {
  ...
  const { data: compromissos = [], isLoading } = useQuery({
    queryKey: ['compromissos-afiliado', user?.email],
    queryFn: async () => {
      if (!user?.email) return [];
      return await base44.entities.Compromisso.filter({ created_by: user.email }, '-data', 200);
    },
    enabled: !!user?.email,
    initialData: [],
  ...
  const createMutation = useMutation({
    ...
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['compromissos-afiliado'] });
        ...
        <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
          <div>
            ...
            <p className="text-slate-600">
              {user?.full_name ? `Agenda de ${user.full_name}` : 'Organize seus compromissos por dia da semana'}
            </p>
          </div>
          <Button
            onClick={() => {
