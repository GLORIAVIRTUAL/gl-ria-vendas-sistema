...
export default function ClientesAfiliado() {
  ...
  const { data: negocios = [] } = useQuery({
    ...
    queryFn: async () => {
      ...
      return await base44.entities.NegocioFechado.filter({ afiliado_id: afiliado.id });
    },
    enabled: !!afiliado?.id,
    initialData: [],
  });

  const negociosIds = useMemo(() => {
    return new Set(negocios.map(n => n.id));
  }, [negocios]);

  const clientes = useMemo(() => {
    return todosClientes.filter(c => negociosIds.has(c.negocio_id));
  }, [todosClientes, negociosIds]);
  ...
  const sincronizarNegociosMutation = useMutation({
    ...
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['clientes-todos'] });
      alert('✅ Seus clientes foram sincronizados com sucesso!');
    },
  });
  ...
  const minhaComissaoMensal = receitaTotal * ((afiliado?.percentual_comissao || 0) / 100);
      ...
      <div className="max-w-7xl mx-auto space-y-6">
        ...
        <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
          <div>
            ...
            <p className="text-slate-600">
              Gestão completa dos seus clientes
            </p>
            {afiliado && (
              <div className="mt-2">
          ...
          >
            ...
            {sincronizarNegociosMutation.isPending ? 'Sincronizando...' : 'Sincronizar Meus Clientes'}
          </Button>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
          ...
          <Card className="p-6 bg-gradient-to-br from-orange-500 to-red-600 text-white shadow-lg">
            ...
            <p className="text-3xl font-bold">R$ {minhaComissaoMensal.toLocaleString('pt-BR', { minimumFractionDigits: 2 })}</p>
              ...
              <TableBody>
                {isLoading ? (
                  ...
                  clientesFiltrados.map((cliente) => {
                    const comissao = (cliente.valor_mensalidade || 0) * ((afiliado?.percentual_comissao || 0) / 100);
